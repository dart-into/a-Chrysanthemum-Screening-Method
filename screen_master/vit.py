...
#from helpers2 import build_model_with_cfg, named_apply, adapt_input_conv
#import PatchEmbed, Mlp, DropPath, trunc_normal_, lecun_normal_
...
default_cfgs = {
    ...
    'vit_small_patch16_224_dino': _cfg(
        ...
        mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), num_classes=0),
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        random_tensor = np.random.rand(1, 3, 224, 224)
        skip = torch.tensor(random_tensor).float().cuda()
        #_x = x
        y = self.patch_embed(skip)
        x = x.flatten(2).transpose(1, 2)  # BCHW -> BNC
